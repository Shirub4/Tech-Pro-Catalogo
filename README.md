# Atualização de GB compatível — sem alterar o banco

Esta versão funciona em bancos antigos mesmo quando as colunas `size_gb` e `total_gb` não existem. Para atualizar um catálogo já configurado, **não execute nenhum SQL**: substitua os arquivos e pressione `Ctrl + F5`. O GB é salvo de forma compatível no campo já existente de informações extras, sem mostrar o marcador interno ao cliente.
