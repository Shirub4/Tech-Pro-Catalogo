...
async function saveOrder(order) {
  ...
  // Não encadeie .select() neste INSERT público.
  // A tabela orders permite que visitantes criem uma seleção, mas somente o
  // administrador autenticado pode lê-la. Solicitar o retorno da linha faria o
  // Supabase aplicar também uma política SELECT e bloquearia visitantes anônimos.
  ...
  if (error && isMissingColumnError(error, "total_gb")) {
    ...
    ({ error } = await supabase.from("orders").insert(compatibleOrder));
  }

  if (error) throw error;

  // O ID real permanece protegido no banco e será visto no painel administrativo.
  // Para o PDF imediato, basta uma referência local sem realizar SELECT público.
  const localId = globalThis.crypto?.randomUUID?.() || `pedido-${Date.now()}`;
  return { ...order, id: localId };
}
